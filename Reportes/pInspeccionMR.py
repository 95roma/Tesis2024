...
class pinspmr(FPDF):
    ...
    def pInspeccionMR(request, id ):
        ...
        try:
            imgpim= ImagenPInspeccionm.objects.get(idpim=pim.id)
        ...
        pdf.cell(w=0,h=6,txt= DMipm.diasestimados if hasattr(DMipm, 'diasestimados') else '', border=1, align='C', fill=False, ln=1)
